<?php
/**
 * Test for FineUploaderField loaded in a page
 * @author James
 */
class FineUploaderUploadCest
{
    ...
    public function uploadTestWorks(AcceptanceTester $I)
    {
        ...
        $path = getenv('DFU_FINEUPLOADER_PATH');
        $I->assertNotEmpty($path, "DFU_FINEUPLOADER_PATH not configured in your .env file!");
        ...
        $I->amOnPage( $path );
        $I->seeElement("form#{$form_id} .fineuploader .qq-uploader.qq-gallery");

        $I->seeElement("form#{$form_id} .fineuploader .dfu-uploader-ui");// UI version of FineUploader

        $config_value = $I->grabAttributeFrom("form#{$form_id} .dfu-uploader-ui", "data-config");
        ...
        /**
         * Uploads: FineUploader would be configured to auto upload in this case
         */
        $uploads = 0;
        for($i=1;$i<=$item_limit;$i++) {
            $I->seeElementInDOM("form#{$form_id} .fineuploader input[name=\"qqfile\"][type=\"file\"]");
            $I->attachFile("form#{$form_id} .fineuploader input[name=\"qqfile\"][type=\"file\"]", "jpg/{$i}.jpg");
            $I->wait(1);
            $I->seeElementInDOM("form#{$form_id} .fineuploader .qq-upload-list li[qq-file-id=\"{$uploads}\"]");// success element
            $uploads++;
            if($i == $item_limit) {
